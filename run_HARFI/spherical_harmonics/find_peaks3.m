...
function [angles,peaks] = find_peaks3(coeff, init_pos, degree)
%%=============================================================
...
for i=1:n
     ...
     for ii=1:n_try
			 [peak_angle] = fminunc(@(x)real_spherical_harmonics(x,coeff(i,:), degree, 1), init_pos(ii,1:2)*pi/180,  options);        
...
angles = reshape(angles,[2 n_try])';
theta = angles(:,1); phi = angles(:,2);
...
peaks = zeros(n,n_try);
for i=1:n_try
    peaks(i) = real_spherical_harmonics([theta(i) phi(i)],coeff(1,:), degree, 1);
end
